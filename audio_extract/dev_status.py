...
def main():
    ...
    parser = argparse.ArgumentParser(description="Check audio extraction processing status")
    parser.add_argument(
        "--db",
        type=Path,
    ...
    parser.add_argument(
        "--recent", type=int, metavar="DAYS", help="Show videos processed in the last N days"
    )
    parser.add_argument(
        "--failed", action="store_true", help="Show only failed processing attempts"
    )
    parser.add_argument("--stats", action="store_true", help="Show processing statistics")
    parser.add_argument(
        "--check", type=Path, help="Check if a specific video file has been processed"
    )
    parser.add_argument(
        "--cleanup", type=int, metavar="DAYS", help="Remove entries older than N days"
    )
    parser.add_argument("--reprocess", type=Path, help="Mark a video for reprocessing")
    ...
    try:
        ...
        if not videos:
            ...
            for video in videos:
                ...
                print(
                    "{:<50} {:<10} {:<20}".format(
                        file_path, video["status"], format_timestamp(video["processed_at"])
                    )
                )
