...
class DriveMonitor:
        ...
        try:
            ...
            if hasattr(self.drive_handler, "upload_file"):
                ...
                audio_file_id = self.drive_handler.upload_file(
                    str(audio_path), file_metadata.get("parents", [None])[0]
                )
            ...
            # Mark as failed
            self.tracker.mark_processed(
                file_id, status="failed", metadata={"file_name": file_name, "error": str(e)}
            )
    ...
    def monitor_continuous(self, folder_id: str, callback: Optional[Callable[[Dict], None]] = None):
