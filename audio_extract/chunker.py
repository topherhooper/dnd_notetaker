...
class AudioChunker:
    ...
    def split(
        ...
        num_chunks = max(
            1, int((duration_seconds + self.chunk_duration - 1) // self.chunk_duration)
        )
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                chunk_path = self._split_chunk(audio_path, temp_dir, start_time, chunk_duration, i)
                ...
                if chunk_path.exists():
                    ...
                    raise ChunkingError(f"Failed to create chunk {i}", chunk_num=i)
    ...
    def _split_chunk(
        ...
        if result.returncode != 0:
            raise ChunkingError(
                f"Failed to split chunk {chunk_num}: {result.stderr}", chunk_num=chunk_num
            )
    ...
    def cleanup(self):
        ...
        for temp_dir_str in self.temp_dirs:
