...
class DriveMonitor:
    ...
    def find_new_recordings(
        ...
        for recording in all_recordings:
            ...
            if not self.tracker.is_processed(file_id):
                new_recordings.append(recording)
        
        logger.info(
            f"Found {len(all_recordings)} total recordings, "
            f"{len(new_recordings)} are new"
        )
    ...
    def check_once(self, folder_id: str, days_back: int = 7) -> Dict[str, int]:
        """Run one check cycle.
    ...
    def monitor(
        ...
        """
        logger.info(
            f"Starting Drive monitoring: folder_id={folder_id}, "
        ...
        while self.running:
            ...
            if self.running:
                logger.info(
                    f"Waiting {self.check_interval}s until next check..."
                )
